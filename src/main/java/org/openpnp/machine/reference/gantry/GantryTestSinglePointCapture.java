...
public final class GantryTestSinglePointCapture {
    ...
    private GantryTestSinglePointCapture() {
    }

    public static String moveAndCaptureFirstPoint(GantryTestCsvInput input) throws Exception {
        if (input == null) {
            throw new Exception("Cannot capture first point because CSV input is null.");
        }

        if (input.getPoints().isEmpty()) {
            throw new Exception("Cannot capture first point because CSV contains no points.");
        }

        GantryTestPoint point = input.getPoints().get(0);

        if (!"Top".equals(point.getTopBottom())) {
            throw new Exception("Step 4.8 only supports Top camera capture. First point Top_Bot="
                    + point.getTopBottom());
        }
        ...
        sb.append("Gantry Test single point capture").append(System.lineSeparator());
        sb.append("REAL MACHINE MOTION WAS REQUESTED.").append(System.lineSeparator());
        sb.append("Only first CSV point will be moved and captured.").append(System.lineSeparator());
        ...
        if (originalImage == null) {
            throw new Exception("Top camera capture returned null image.");
        }
        ...
        if (cropImage == null) {
            throw new Exception("Cannot create centered mono crop. Crop_factor="
                    + point.getCropFactor()
                    + ", image width="
                    + monoImage.getWidth()
                    + ", image height="
        ...
        String baseFileName = buildBaseFileName(point);
        ...
        sb.append("Gantry Test single point capture PASSED.");
    ...
    private static Path createOutputFolder(GantryTestCsvInput input) throws Exception {
        ...
        Path outputFolder = baseFolder.resolve(LocalDateTime.now().format(OUTPUT_FOLDER_TIMESTAMP));
    ...
    private static String buildBaseFileName(GantryTestPoint point) {
        return String.format(Locale.US,
                "Cycle_001_Point_%03d_Line_%03d_Top",
                point.getIndex(),
                point.getLineNumber());
    }
