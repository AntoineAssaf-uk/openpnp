...
public final class GantryTestSingleCycleCapture {
    ...
    private GantryTestSingleCycleCapture() {
    }

    public static String moveAndCaptureCycleOne(GantryTestCsvInput input) throws Exception {
        if (input == null) {
            throw new Exception("Cannot capture cycle 1 because CSV input is null.");
        }

        if (input.getPoints().isEmpty()) {
            throw new Exception("Cannot capture cycle 1 because CSV contains no points.");
        }
        ...
        sb.append("Gantry Test cycle 1 capture").append(System.lineSeparator());
        ...
        for (GantryTestPoint point : input.getPoints()) {
            ...
            sb.append(String.format(Locale.US,
                    "Point visit %d / %d: line=%d, #=%d, Name=\"%s\"",
                    pointVisit,
            ...
            if (originalImage == null) {
                throw new Exception("Top camera capture returned null image at line "
                        + point.getLineNumber() + ".");
            }
            ...
            if (cropImage == null) {
                throw new Exception("Cannot create centered mono crop at line "
                        + point.getLineNumber()
                        + ". Crop_factor="
                        + point.getCropFactor()
                        + ", image width="
    ...
    private static Path createOutputFolder(GantryTestCsvInput input) throws Exception {
        ...
        Path outputFolder = baseFolder.resolve("Cycle_001_" + LocalDateTime.now().format(OUTPUT_FOLDER_TIMESTAMP));
